...
from .typed_materializer import binding_key
from .types import MaterializedBinding, SQLiteProgram, SchemaInventory
...
def compile_sqlite_program(ir: dict[str, Any], inventory: SchemaInventory, values: dict[str, MaterializedBinding]) -> SQLiteProgram:
    operation = ir["operation"]
    table = quote_identifier(table_name(inventory, ir["table_ref"]))
    if operation == "INSERT":
        cols, params = _assignment_columns_and_params(ir["assignments"], inventory, values, "assignments")
        ...
        insert_cols, insert_params = _assignment_columns_and_params(ir["insert_assignments"], inventory, values, "insert_assignments")
        ...
        if ir["update_policy"] == "DO_NOTHING":
            ...
            update_cols, update_params = _assignment_columns_and_params(ir["update_assignments"], inventory, values, "update_assignments")
...
def _assignment_columns_and_params(assignments: list[dict[str, str]], inventory: SchemaInventory, values: dict[str, MaterializedBinding], context: str) -> tuple[list[str], tuple[Any, ...]]:
    cols: list[str] = []
    params: list[Any] = []
    for index, item in enumerate(assignments):
        cols.append(quote_identifier(column(inventory, item["column_ref"]).name))
        params.append(values[binding_key(context, index)].value)
    return cols, tuple(params)


def _where(selector: dict[str, Any], inventory: SchemaInventory, values: dict[str, MaterializedBinding]) -> tuple[str, tuple[Any, ...]]:
    parts: list[str] = []
    params: list[Any] = []
    connector = f" {selector['connector']} "
    for index, item in enumerate(selector["predicates"]):
        parts.append(f"{quote_identifier(column(inventory, item['column_ref']).name)} {OPERATOR_SQL[item['operator']]} ?")
        params.append(values[binding_key("row_selector.predicates", index)].value)

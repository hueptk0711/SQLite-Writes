...
from nldbwrite_v3.v2_a1.typed_materializer import enforce_unique_non_omit_span_refs  # noqa: E402
...
def evaluate_method(method_id: str, row: dict[str, Any], stage_dir: Path, raw: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if method_id == "M2_FROZEN_A7":
        try:
            ...
            enforce_unique_non_omit_span_refs(dict(phase_o.get("column_span_refs") or {}))
            ...
            build_status = "builder_error" if parse_status == "success" else "success"
            parsed.update(
                {
            ...
            evaluation = evaluate_sql(row, stage_dir, [], parse_status=parse_status, build_status=build_status)
            evaluation["error_type"] = failure_stage
            return parsed, evaluation
        except Exception as exc:
            parsed.update({"parse_status": "success", "failure_stage": "COMPILATION_FAILURE", "diagnostics": [str(exc)]})
            evaluation = evaluate_sql(row, stage_dir, [], build_status="builder_error")
            evaluation["error_type"] = "COMPILATION_FAILURE"
            return parsed, evaluation
    raise KeyError(method_id)
...
def failure_stage_from_v2a1_error(exc: V2A1Error) -> str:
    ...
    if code == "duplicate_span_ref" or code == "completeness_duplicate_slot":

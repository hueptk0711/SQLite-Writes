#!/usr/bin/env python3
"""Stage7E0-A6 one-call column-conditioned real-generation preflight runner."""
...
A6_PRIMARY_SET_REL = f"{STAGE7C_A6_DIR}/FRESH_ENGLISH_A6_PRIMARY_FEASIBILITY_SET.jsonl"
A6_DIAGNOSTIC_SET_RELS = [
    f"{STAGE7C_A6_DIR}/A5_OBSERVED_REGRESSION_DIAGNOSTICS_A6.jsonl",
    f"{STAGE7C_A6_DIR}/A6_METHOD_STRESS_REGRESSION_DIAGNOSTICS_A6.jsonl",
    f"{STAGE7C_A6_DIR}/REVIEWER_GUIDED_A6_STRESS_DIAGNOSTICS.jsonl",
]
A6_PROMPT_SPEC_REL = f"{STAGE7C_A6_DIR}/ATOMIC_DOMAIN_COLUMN_CONDITIONED_PROMPT_SPEC_A6_ENGLISH.json"
STAGE7E0_A6_INPUT_MANIFEST_REL = f"{STAGE_NAME}/STAGE7E0_A6_INPUT_MANIFEST.json"
EXPECTED_PRIMARY_COUNT = 12
EXPECTED_DIAGNOSTIC_COUNT = 36
...
def verify_accepted_protocol_commit_argument(accepted_commit: str) -> dict[str, Any]:
    manifest_path = PROJECT_ROOT / STAGE7E0_A6_INPUT_MANIFEST_REL
    if not manifest_path.is_file():
        raise SystemExit(f"STOP: missing frozen Stage7E0-A6 input manifest: {STAGE7E0_A6_INPUT_MANIFEST_REL}")
    manifest = read_json(manifest_path)
    frozen = manifest.get("accepted_stage7c_a6_commit")
    if not isinstance(frozen, str) or not frozen:
        raise SystemExit("STOP: frozen Stage7E0-A6 input manifest does not record accepted_stage7c_a6_commit")
...
def assert_git_lock(accepted_commit: str) -> dict[str, Any]:
    ...
    if head != accepted_commit:
        raise SystemExit(f"STOP: git HEAD {head} != accepted Stage7C-A6 protocol commit {accepted_commit}")
...
def validate_generation_config(args: argparse.Namespace) -> None:
    ...
    if backend == "hf":
        raise SystemExit("STOP: backend=hf is plain unconstrained HF and is forbidden for Stage7E0-A6")
    ...
    if backend == "constrained_hf" and str(args.quantization).lower() not in {"none", ""}:
        raise SystemExit("STOP: Stage7E0-A6 forbids quantized generation")
    if backend == "constrained_hf" and bool(args.resume):
        raise SystemExit("STOP: Stage7E0-A6 forbids --resume; archive partial output and start a fresh result-root")
...
def verify_stage7c_a6_lock(root: Path = PROJECT_ROOT) -> dict[str, Any]:
    lock = read_json(root / STAGE7C_A6_DIR / "STAGE7C_A6_LOCK.json")
    prompt = read_json(root / A6_PROMPT_SPEC_REL)
    token_audit = read_json(root / STAGE7C_A6_DIR / "FULL_RENDERED_PROMPT_TOKEN_AUDIT.json")
    if lock.get("status") != "PASS_ATOMIC_DOMAIN_COLUMN_CONDITIONED_PROTOCOL_FROZEN":
        raise SystemExit("STOP: Stage7C-A6 protocol is not closed/pass")
    if lock.get("phase_m_primary_pipeline_removed") is not True or lock.get("model_generates_phase_m") is not False:
        raise SystemExit("STOP: Stage7C-A6 Phase M removal lock drifted")
    if prompt.get("model_id") != MODEL_ID or prompt.get("model_revision") != MODEL_REVISION:
        raise SystemExit("STOP: Stage7C-A6 model identity drifted")
    if token_audit.get("tokenizer_status") != "PASS" or token_audit.get("chat_template_hash_matches_required") is not True:
        raise SystemExit("STOP: Stage7C-A6 tokenizer/chat-template audit is not PASS")
    return {
        "stage7c_a6_lock_sha256": sha256_file(root / STAGE7C_A6_DIR / "STAGE7C_A6_LOCK.json"),
        "a6_prompt_spec_sha256": sha256_file(root / A6_PROMPT_SPEC_REL),
        "a6_primary_set_sha256": sha256_file(root / A6_PRIMARY_SET_REL),
        "a6_diagnostic_set_sha256": sha256_text(canonical_json([sha256_file(root / rel) for rel in A6_DIAGNOSTIC_SET_RELS])),
        "tokenizer_status": token_audit["tokenizer_status"],
        "chat_template_sha256": token_audit["chat_template_sha256"],
    }


def load_stage7c_a6_rows(root: Path = PROJECT_ROOT, *, diagnostics: bool = False) -> list[dict[str, Any]]:
    rows = [row for rel in A6_DIAGNOSTIC_SET_RELS for row in read_jsonl(root / rel)] if diagnostics else read_jsonl(root / A6_PRIMARY_SET_REL)
    expected_count = EXPECTED_DIAGNOSTIC_COUNT if diagnostics else EXPECTED_PRIMARY_COUNT
    expected_prefixes = (
        ("stage7c_a5_primary_english_", "stage7c_a6_method_stress_english_", "stage7c_a6_reviewer_guided_english_")
        if diagnostics
        else ("stage7c_a6_primary_english_",)
    )
    if len(rows) != expected_count:
        raise SystemExit(f"STOP: expected {expected_count} A6 rows, found {len(rows)}")
    for row in rows:
        if not str(row.get("sample_id")).startswith(expected_prefixes):
            raise SystemExit(f"STOP: A6 row id prefix drifted for {row.get('sample_id')}")
        if diagnostics and row.get("diagnostic_role") != "diagnostic_only_after_primary":
            raise SystemExit(f"STOP: A6 diagnostic role drifted for {row.get('sample_id')}")
        ...
        if sorted(row["label_side_expected"]["phase_o"]) != ["column_span_refs", "operation", "table_ref"]:
            raise SystemExit(f"STOP: A6 Phase O label contract drifted for {row.get('sample_id')}")
    return rows
...
def _enum_values(node: dict[str, Any]) -> list[str]:
    ...
    if not isinstance(values, list) or not values:
        raise V2A1Error("constraint_schema_enum_missing", "A6 constrained generation requires finite enum/const choices")
...
class ColumnConditionedConstraintGrammar:
    ...
    def metadata(self) -> dict[str, Any]:
        return {
            ...
            "constraint_source": "A6_dynamic_column_conditioned_json_schema",
...
def generate_constrained_a6(model: Any, tokenizer: Any, messages: list[dict[str, str]], *, max_new_tokens: int, schema: dict[str, Any]) -> dict[str, Any]:
    import torch
...
class ConstrainedTransformersChatGenerator:
    def __init__(self, *, model_name_or_path: str, quantization: str, trust_remote_code: bool, max_input_tokens: int, seed: int):
        ...
        if quantization not in {"none", "None", ""}:
            raise SystemExit("STOP: Stage7E0-A6 forbids 4-bit or any quantized generation")
        self.torch = torch
        self.seed = seed
        ...
        if sha256_text(getattr(self.tokenizer, "chat_template", None) or "") != EXPECTED_CHAT_TEMPLATE_SHA256:
            raise SystemExit("STOP: tokenizer chat_template hash does not match Stage7C-A6 lock")
    ...
    def generate(self, *, sample_id: str, messages: list[dict[str, str]], max_new_tokens: int, row: dict[str, Any]) -> CallResult:
        ...
        generated = generate_constrained_a6(self.model, self.tokenizer, messages, max_new_tokens=max_new_tokens, schema=row["runtime_constraints"]["phase_o_schema"])
        return CallResult(
            sample_id=sample_id,
            phase="phase_o",
...
def branch_for_table(schema: dict[str, Any], table_ref: str) -> dict[str, Any]:
    for branch in _branch_specs(schema):
        if branch["table_ref"] == table_ref:
            return branch
    raise V2A1Error("phase_o_unknown_table_ref", "Unknown table_ref for A6 schema", details={"table_ref": table_ref})
...
def parse_phase_o_column_conditioned_output(raw: str, schema: dict[str, Any]) -> dict[str, Any]:
    ...
    if set(obj) != {"operation", "table_ref", "column_span_refs"}:
        raise V2A1Error("phase_o_schema_failure", "A6 output must contain only operation, table_ref, column_span_refs")
    if obj["operation"] != "INSERT":
        raise V2A1Error("phase_o_schema_failure", "A6 operation must be INSERT")
...
def evaluate_case(row: dict[str, Any], generator: OneCallGenerator, *, phase_o_max_new_tokens: int) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    try:
        ...
        reason = exc.reason_code if isinstance(exc, V2A1Error) else "A6_deterministic_oracle"
...
def run_stage7e0(args: argparse.Namespace) -> dict[str, Any]:
    ...
    A6_lock = verify_stage7c_a6_lock()
    primary_rows = load_stage7c_a6_rows()
    if result_root.exists() and not args.resume:
        raise SystemExit("STOP: result-root already exists; do not reuse real A6 result directories")
    result_root.mkdir(parents=True, exist_ok=True)
    if backend == "mock":
        generator: OneCallGenerator = LabelMockGenerator(primary_rows + load_stage7c_a6_rows(diagnostics=True))
    else:
    ...
    if backend == "constrained_hf" and metadata.get("cuda_available") is not True:
        raise SystemExit("STOP: real Stage7E0-A6 generation requires cuda_available=true")
    ...
    write_json(
        result_root / "run_manifest.json",
        {
            ...
            "phase_o_prompt_spec_path": A6_PROMPT_SPEC_REL,
    ...
    if args.run_diagnostics_after_primary_pass:
        if pass_count != EXPECTED_PRIMARY_COUNT:
            raise SystemExit("STOP: Stage7E0-A6 primary failed; do not run diagnostics or open Gretel pilot")
        diagnostic_rows = load_stage7c_a6_rows(diagnostics=True)
    ...
    if backend == "constrained_hf" and summary["status"] != "PASS":
        print("Stage7E0-A6 primary gate failed; Gretel and diagnostics remain closed.", file=sys.stderr)
    return summary

...
from scripts.server.run_stage7e0_a5_english import (  # noqa: E402
    ...
    STAGE_NAME,
    STAGE7E0_A5_INPUT_MANIFEST_REL,
...
def _load_expected_input_manifest() -> tuple[dict[str, Any], list[str]]:
    path = PROJECT_ROOT / STAGE7E0_A5_INPUT_MANIFEST_REL
    if not path.is_file():
        return {}, [f"missing_stage7e0_a5_input_manifest:{STAGE7E0_A5_INPUT_MANIFEST_REL}"]
    try:
        return read_json(path), []
    except Exception as exc:
        return {}, [f"could_not_parse_stage7e0_a5_input_manifest:{exc}"]
...
def _expected_rows() -> tuple[list[dict[str, Any]], list[str]]:
    try:
        ...
        return [], [f"could_not_load_locked_a5_primary_rows:{exc}"]
    return rows, []
...
def _safe_read_result(result_dir: Path) -> tuple[dict[str, Any], dict[str, Any], list[dict[str, Any]], list[dict[str, Any]], list[str]]:
    ...
    if (result_dir / "raw_phase_m_generations.jsonl").exists():
        failures.append("phase_m_raw_generations_must_not_exist_for_a5")
    try:
        return (
...
def evidence_integrity_failures(result_dir: Path, summary: dict[str, Any], manifest: dict[str, Any], cases: list[dict[str, Any]], raw_o: list[dict[str, Any]], frozen_rows: list[dict[str, Any]], expected_input: dict[str, Any]) -> list[str]:
    ...
    if list(case_by_id) != frozen_ids:
        failures.append("primary_case_result_ids_must_equal_locked_a5_primary_ids")
    if list(raw_by_id) != frozen_ids:
        failures.append("raw_generation_ids_must_equal_locked_a5_primary_ids")
    for raw in raw_o:
    ...
    if summary.get("gretel_pilot_opened") is not False:
        failures.append("gretel_pilot_must_remain_unopened_during_stage7e0_a5")
    if manifest.get("phase_o_prompt_spec_path") != A5_PROMPT_SPEC_REL:
        failures.append("server_did_not_use_exact_a5_phase_o_prompt_path")
    frozen_commit = expected_input.get("accepted_stage7c_a5_commit")
    ...
    observed_inputs = manifest.get("stage7c_a5_inputs", {})
    expected_input_hashes = {
        "stage7c_a5_lock_sha256": expected_input.get("stage7c_a5_lock_sha256"),
        "a5_prompt_spec_sha256": expected_input.get("phase_o_prompt_spec_sha256"),
        "a5_primary_set_sha256": expected_input.get("primary_set_sha256"),
        "a5_diagnostic_set_sha256": expected_input.get("diagnostic_set_sha256"),
    ...
    for key, expected in expected_input_hashes.items():
        if observed_inputs.get(key) != expected:
            failures.append(f"run_manifest_stage7c_a5_input_mismatch:{key}")

#!/usr/bin/env python3
"""Validate Stage7C-A6 column-conditioned Phase O protocol artifacts."""
...
def validate_prompt_and_schema(stage_dir: Path, failures: list[str]) -> None:
    prompt = read_json(stage_dir / "ATOMIC_DOMAIN_COLUMN_CONDITIONED_PROMPT_SPEC_A6_ENGLISH.json")
    output = read_json(stage_dir / "ATOMIC_DOMAIN_COLUMN_CONDITIONED_OUTPUT_SPEC_A6.json")
    runtime = read_json(stage_dir / "ATOMIC_DOMAIN_COLUMN_CONDITIONED_RUNTIME_SCHEMA_SPEC_A6.json")
    serialization = read_json(stage_dir / "ATOMIC_DOMAIN_COLUMN_CONDITIONED_SERIALIZATION_FREEZE.json")
    branching = read_json(stage_dir / "TARGET_TABLE_BRANCHING_PROTOCOL_A6.json")
    no_phase_m = read_json(stage_dir / "NO_PHASE_M_PRIMARY_PIPELINE_SPEC_A6.json")
    failure_policy = read_json(stage_dir / "OMIT_AND_CANDIDATE_MISS_FAILURE_POLICY_A6.json")
    evaluator = read_json(stage_dir / "EVALUATOR_SEMANTICS_A6.json")
    ...
    if runtime.get("candidate_domain_filter_enabled") is not True:
        failures.append("runtime_candidate_domain_filter_not_enabled")
    if serialization.get("candidate_domain_filter_source_stage") is None or serialization.get("candidate_domain_filter_source_patch") != "PATCH2":
        failures.append("serialization_candidate_domain_filter_source_mismatch")
...
def validate_rows(
    ...
    *,
    ...
    require_coverage_tags: bool = True,
...
def validate_manifests(stage_dir: Path, failures: list[str]) -> None:
    ...
    lock = read_json(stage_dir / "STAGE7C_A6_LOCK.json")
    ...
    if lock.get("candidate_domain_filter_enabled") is not True or lock.get("candidate_domain_gold_suppressed_total") != 0:
        failures.append("lock_candidate_domain_filter_mismatch")
    if domain_audit.get("status") != "PASS" or domain_audit.get("gold_suppressed_total") != 0:
        failures.append("candidate_domain_audit_not_pass")
    if domain_audit.get("suppressed_candidate_total", 0) <= 0 or domain_audit.get("unfiltered_candidate_total", 0) <= domain_audit.get("filtered_candidate_total", 0):
        failures.append("candidate_domain_audit_no_suppression")
    if runtime_freeze.get("source_patch") != "PATCH2" or runtime_freeze.get("manual_alias_additions_allowed_after_a6_outputs") is not False:
        failures.append("candidate_domain_runtime_freeze_mismatch")
    if runtime_freeze.get("candidate_miss_policy", "").find("method failure") < 0:
        failures.append("candidate_domain_runtime_freeze_policy_missing")
...
def validate(stage_dir: Path, *, rebuild: bool = False, tokenizer_name_or_path: str | None = None, tokenizer_revision: str = QWEN_TOKENIZER_REVISION) -> dict[str, Any]:
    ...
    case_count, assigned_count, omit_count, multi_table_count = validate_rows(
        stage_dir,
        failures,
        rows_name="FRESH_ENGLISH_A6_PRIMARY_FEASIBILITY_SET.jsonl",
        oracle_name="ORACLE_ATOMIC_DOMAIN_COLUMN_CONDITIONED_PRIMARY_RESULTS.jsonl",
        expected_prefix="stage7c_a6_primary_english_",
    )
    diagnostic_case_count, diagnostic_assigned_count, diagnostic_omit_count, diagnostic_multi_table_count = validate_rows(
        stage_dir,
        failures,
        rows_name="A5_OBSERVED_REGRESSION_DIAGNOSTICS_A6.jsonl",
        oracle_name="ORACLE_A5_OBSERVED_DIAGNOSTIC_RESULTS.jsonl",

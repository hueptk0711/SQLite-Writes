#!/usr/bin/env python3
"""Import Stage7E0-A4 Kaggle primary results into a reviewer evidence package."""
...
from scripts.data.validate_stage7e0_a4_server_results import classify_result

STAGE_NAME = "Stage7E0_A4_ENGLISH_CANDIDATE_SPAN_REAL_GENERATION_PREFLIGHT"
PATCH_NAME = "PATCH4"
SERVER_RUN_ID = "kaggle_primary_run_20260901_064812_patch3_t4x2"
...
SERVER_TAR_NAME = "stage7e0_a4_patch3_kaggle_primary_result_20260901_064812.tar.gz"
...
def find_result_dir(result_root: Path) -> Path:
    ...
    if len(candidates) == 1:
        return candidates[0]
    raise RuntimeError(f"Expected one Stage7E0-A4 result directory inside {result_root}")
...
def decision_from_classification(classification: dict[str, Any]) -> str:
    ...
    if classification["protocol_compliance_status"] != "PASS":
        return "Server output is preserved as protocol-invalid evidence; do not use it as a scientific A4 result."
    if classification["primary_gate_status"] == "PASS":
        return "Protocol-compliant constrained A4 output passed 10/10; Gretel pilot eligibility can be reviewed in the next stage."
    return "Protocol-compliant constrained A4 output is a scientific primary failure; do not open Gretel pilot."
...
def refresh_derived_manifest(stage_dir: Path, result_root: Path) -> None:
    ...
    rel_paths.update(
        {
            ...
            "STAGE7E0_A4_SERVER_RESULT_LOCK.json",
    ...
    lock_path = stage_dir / "STAGE7E0_A4_LOCK.json"
...
def import_server_results(stage_dir: Path, tar_path: Path) -> dict[str, Any]:
    ...
    write_json(stage_dir / "STAGE7E0_A4_SERVER_RESULT_LOCK.json", lock)
...
def validation_report(report: dict[str, Any]) -> str:
    ...
    status = status_from_classification(classification)
    decision = decision_from_classification(classification)
    return f"""# Stage7E0-A4 English PATCH4 Kaggle Primary Result Validation Report
...
def reviewer_readme(report: dict[str, Any]) -> str:
    classification = report["server_result_classification"]
    return f"""# Stage7E0-A4 English Kaggle Primary Result PATCH4
...
{STAGE_NAME}/STAGE7E0_A4_SERVER_RESULT_LOCK.json
```
"""
...
def include_paths(stage_dir: Path, tar_path: Path) -> list[Path]:
    rel_paths = [
        ...
        "requirements-inference-kaggle-t4x2.lock.txt",
        ...
        "scripts/data/import_stage7e0_a4_server_results.py",
        ...
        "tests/test_stage7e0_a4_server_results.py",
        "tests/test_stage7c_a4_candidate_span_phase_o_protocol.py",
        "tests/test_stage7b_a2_candidate_span_reference.py",
    ...
    tar_sha = Path(str(tar_path) + ".sha256")
    if tar_sha.is_file():

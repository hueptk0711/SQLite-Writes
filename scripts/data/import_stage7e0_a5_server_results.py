#!/usr/bin/env python3
"""Import Stage7E0-A5 UET primary results into a reviewer evidence package."""
...
SERVER_RESULT_CLASSIFICATION_NAME = "SERVER_RESULT_CLASSIFICATION_PATCH5.json"
SERVER_RESULT_VALIDATION_REPORT_NAME = "VALIDATION_REPORT_PATCH5.md"
...
def find_result_dir(result_root: Path) -> Path:
    ...
    if len(candidates) == 1:
        return candidates[0]
    raise RuntimeError(f"Expected one Stage7E0-A5 result directory inside {result_root}")
...
def decision_from_classification(classification: dict[str, Any]) -> str:
    ...
    if classification["protocol_compliance_status"] != "PASS":
        return "Server output is preserved as protocol-invalid evidence; do not use it as a scientific A5 result."
    if classification["primary_gate_status"] == "PASS":
        return "Protocol-compliant constrained A5 output passed 12/12; Gretel pilot eligibility can be reviewed in the next stage."
    return "Protocol-compliant constrained A5 output is a scientific primary failure; do not open Gretel pilot."
...
def refresh_derived_manifest(stage_dir: Path, result_root: Path) -> None:
    ...
    rel_paths.update(
        {
            ...
            "STAGE7E0_A5_SERVER_RESULT_LOCK.json",
    ...
    lock_path = stage_dir / "STAGE7E0_A5_LOCK.json"
...
def import_server_results(stage_dir: Path, tar_path: Path) -> dict[str, Any]:
    ...
    write_json(stage_dir / "STAGE7E0_A5_SERVER_RESULT_LOCK.json", lock)
...
def validation_report(report: dict[str, Any]) -> str:
    ...
    status = status_from_classification(classification)
    decision = decision_from_classification(classification)
    return f"""# Stage7E0-A5 English PATCH5 UET Primary Result Validation Report
...
def reviewer_readme(report: dict[str, Any]) -> str:
    classification = report["server_result_classification"]
    return f"""# Stage7E0-A5 English UET Primary Result PATCH5
...
{STAGE_NAME}/STAGE7E0_A5_SERVER_RESULT_LOCK.json
```
"""
...
def include_paths(stage_dir: Path, tar_path: Path) -> list[Path]:
    ...
    for rel in (
        "scripts/data/import_stage7e0_a5_server_results.py",
        "tests/test_stage7e0_a5_server_results.py",
    ):

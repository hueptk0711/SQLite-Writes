#!/usr/bin/env python3
"""Import Stage7E0-A6 UET primary results into a reviewer evidence package."""
...
from scripts.data.build_stage7e0_a6_english_preflight import include_paths as preflight_include_paths  # noqa: E402
from scripts.data.validate_stage7e0_a6_server_results import classify_result  # noqa: E402


STAGE_NAME = "Stage7E0_A6_ENGLISH_ATOMIC_DOMAIN_COLUMN_CONDITIONED_REAL_GENERATION_PREFLIGHT"
PATCH_NAME = "PATCH1"
PACKAGE_DATE = "20260902"
SERVER_RUN_ID = "uet_p4"
...
SERVER_RESULT_CLASSIFICATION_NAME = "SERVER_RESULT_CLASSIFICATION_PATCH1.json"
SERVER_RESULT_VALIDATION_REPORT_NAME = "VALIDATION_REPORT_PATCH1.md"
...
def find_result_dir(result_root: Path) -> Path:
    ...
    if len(candidates) == 1:
        return candidates[0]
    raise RuntimeError(f"Expected one Stage7E0-A6 result directory inside {result_root}")
...
def decision_from_classification(classification: dict[str, Any]) -> str:
    ...
    if classification["protocol_compliance_status"] != "PASS":
        return "Server output is preserved as protocol-invalid evidence; do not use it as a scientific A6 result."
    if classification["primary_gate_status"] == "PASS":
        return "Protocol-compliant constrained A6 output passed 12/12; Gretel pilot eligibility can be reviewed in the next stage."
    return "Protocol-compliant constrained A6 output is a scientific primary failure; do not open Gretel pilot."
...
def refresh_derived_manifest(stage_dir: Path, result_root: Path) -> None:
    ...
    if manifest_path.is_file():
        rel_paths.update(item["path"] for item in read_json(manifest_path).get("artifacts", []))
    rel_paths.update(
        {
            "SERVER_RESULT_FAILURE_ANALYSIS.md",
            "SERVER_RESULT_IMPORT_REPORT.json",
            "STAGE7E0_A6_SERVER_RESULT_LOCK.json",
    ...
    lock_path = stage_dir / "STAGE7E0_A6_LOCK.json"
...
def import_server_results(stage_dir: Path, tar_path: Path) -> dict[str, Any]:
    ...
    raw_o = read_jsonl(extracted / "raw_primary_phase_o_generations.jsonl")
    ...
    report = {
        ...
        "source_tar_sha256_file": {
            "path": f"{tar_path.name}.sha256",
            "sha256": sha256_file(Path(f"{tar_path}.sha256")) if Path(f"{tar_path}.sha256").is_file() else None,
        },
        "server_result_files": {
            ...
            "raw_primary_phase_o_generations.jsonl": sha256_file(extracted / "raw_primary_phase_o_generations.jsonl"),
        ...
        "server_result_classification": {
            ...
            "runtime_profile_id": manifest.get("model", {}).get("runtime_profile_id"),
            "primary_runtime_profile_id": manifest.get("model", {}).get("primary_runtime_profile_id"),
    ...
    write_text(stage_dir / "SERVER_RESULT_FAILURE_ANALYSIS.md", failure_analysis(report, cases, raw_o))
    write_text(stage_dir / SERVER_RESULT_VALIDATION_REPORT_NAME, validation_report(report))
    write_text(stage_dir / "REVIEWER_README.md", reviewer_readme(report))
    ...
    write_json(stage_dir / "STAGE7E0_A6_SERVER_RESULT_LOCK.json", lock)
    refresh_derived_manifest(stage_dir, result_root)
    return report


def failure_analysis(report: dict[str, Any], cases: list[dict[str, Any]], raw_o: list[dict[str, Any]]) -> str:
    ...
    lines = [
        ...
        f"protocol_backend={report['result']['protocol_backend']}",
...
def validation_report(report: dict[str, Any]) -> str:
    ...
    status = status_from_classification(classification)
    decision = decision_from_classification(classification)
    return f"""# Stage7E0-A6 English PATCH1 UET Primary Result Validation Report
...
protocol_backend={report["result"]["protocol_backend"]}
...
def reviewer_readme(report: dict[str, Any]) -> str:
    classification = report["server_result_classification"]
    return f"""# Stage7E0-A6 English UET Primary Result PATCH1
...
```text
{STAGE_NAME}/{SERVER_RESULT_VALIDATION_REPORT_NAME}
{STAGE_NAME}/SERVER_RESULT_FAILURE_ANALYSIS.md
{STAGE_NAME}/SERVER_RESULT_IMPORT_REPORT.json
{STAGE_NAME}/STAGE7E0_A6_SERVER_RESULT_LOCK.json
```
"""


def include_paths(stage_dir: Path, tar_path: Path) -> list[Path]:
    paths = preflight_include_paths(stage_dir)
    paths.extend([path for path in stage_dir.rglob("*") if path.is_file()])
    if tar_path.is_file():
        paths.append(tar_path)
    tar_sha = Path(f"{tar_path}.sha256")
    if tar_sha.is_file():
        paths.append(tar_sha)
    for rel in (
        "scripts/data/import_stage7e0_a6_server_results.py",
        "tests/test_stage7e0_a6_server_results.py",
    ):
...
def package_reviewer(stage_dir: Path, tar_path: Path, package_path: Path) -> str:
    ...
    with zipfile.ZipFile(package_path, "w", compression=zipfile.ZIP_DEFLATED) as archive:
        for path in include_paths(stage_dir, tar_path):
            ...
            elif path == Path(f"{tar_path}.sha256"):

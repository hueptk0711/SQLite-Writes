#!/usr/bin/env python3
"""Validate Stage7E0-A6 one-call column-conditioned preflight artifacts."""
...
from scripts.data.build_stage7e0_a6_english_preflight import (  # noqa: E402
...
def validate(stage_dir: Path) -> dict[str, Any]:
    ...
    if upstream.get("status") != "PASS":
        failures.append("stage7c_A6_upstream_validation_failed")

    protocol = read_json(stage_dir / "RUNNER_PROTOCOL_A6.json")
    inputs = read_json(stage_dir / "STAGE7E0_A6_INPUT_MANIFEST.json")
    policy = read_json(stage_dir / "PRIMARY_ACCEPTANCE_POLICY_A6.json")
    independence = read_json(stage_dir / "CONSTRAINT_INDEPENDENCE_AUDIT_A6.json")
    ...
    if prompt_contract.get("phase_o_prompt_spec_path") != A6_PROMPT_SPEC_REL:
    ...
    if inputs.get("phase_o_prompt_spec_path") != A6_PROMPT_SPEC_REL:
    ...
    if inputs.get("phase_o_prompt_spec_sha256") != sha256_file(PROJECT_ROOT / A6_PROMPT_SPEC_REL):
        failures.append("input_manifest_prompt_hash_mismatch")
    if inputs.get("primary_set_sha256") != sha256_file(PROJECT_ROOT / STAGE7C_A6_DIR / "FRESH_ENGLISH_A6_PRIMARY_FEASIBILITY_SET.jsonl"):
    ...
    if lock.get("status") != "PASS_READY_FOR_REAL_A6_CONSTRAINED_PREFLIGHT":
    ...
    if lock.get("constraint_independence_audit_sha256") != sha256_file(stage_dir / "CONSTRAINT_INDEPENDENCE_AUDIT_A6.json"):
    ...
    for needle in ("UET", SERVER_REQUIREMENTS_LOCK, "CUDA_VISIBLE_DEVICES=0", "preflight_runtime_stage7e0_a6.py", PRIMARY_RUNTIME_PROFILE_ID, "run_stage7e0_a6_english.py", "--backend constrained_hf", "--quantization none", "--phase-o-max-new-tokens 512", PRIMARY_RESULT_DIR_NAME, "Do not use `--resume`", "less than 12/12", "validate", "archive", "sha256"):
    ...
    run_pos = commands.find("python scripts/server/run_stage7e0_a6_english.py")
    validate_pos = commands.find("python scripts/data/validate_stage7e0_a6_server_results.py")

#!/usr/bin/env python3
"""Build Stage7E0-A6 one-call column-conditioned preflight artifacts."""
...
from scripts.data.validate_stage7c_a6_atomic_domain_column_conditioned_protocol_freeze import validate as validate_stage7c_a6  # noqa: E402
...
from scripts.data.build_stage7c_a6_atomic_domain_column_conditioned_protocol_freeze import canonical_json, sha256_text  # noqa: E402
...
PRIMARY_RESULT_DIR_NAME = "stage7e0_a6_english_atomic_domain_column_conditioned_uet_rtx4090_primary_results_20260902"
DIAGNOSTIC_RESULT_DIR_NAME = "stage7e0_a6_english_atomic_domain_column_conditioned_uet_rtx4090_diagnostic_results_20260902"
...
def frozen_accepted_stage7c_a6_commit() -> str:
    existing_manifest = PROJECT_ROOT / STAGE_NAME / "STAGE7E0_A6_INPUT_MANIFEST.json"
    if existing_manifest.is_file():
        value = read_json(existing_manifest).get("accepted_stage7c_a6_commit")
        if isinstance(value, str) and value:
            return value
    committed_manifest = git_output("show", f"HEAD:{STAGE_NAME}/STAGE7E0_A6_INPUT_MANIFEST.json")
    if committed_manifest:
        value = json.loads(committed_manifest).get("accepted_stage7c_a6_commit")
        if isinstance(value, str) and value:
            return value
    return git_output("rev-parse", "HEAD") or "UNKNOWN"


def stage7c_a6_inputs(accepted_commit: str) -> dict[str, Any]:
    upstream = validate_stage7c_a6(PROJECT_ROOT / STAGE7C_A6_DIR)
    if upstream.get("status") != "PASS":
        raise RuntimeError(f"Stage7C-A6 validation must pass before Stage7E0-A6: {upstream.get('failures')}")
    primary_rows = load_stage7c_a6_rows(PROJECT_ROOT)
    diagnostic_rows = load_stage7c_a6_rows(PROJECT_ROOT, diagnostics=True)
    token_audit = read_json(PROJECT_ROOT / STAGE7C_A6_DIR / "FULL_RENDERED_PROMPT_TOKEN_AUDIT.json")
    return {
        "stage7c_a6_dir": STAGE7C_A6_DIR,
        "accepted_stage7c_a6_commit": accepted_commit,
        "phase_o_prompt_spec_path": A6_PROMPT_SPEC_REL,
        "phase_o_prompt_spec_sha256": sha256_file(PROJECT_ROOT / A6_PROMPT_SPEC_REL),
        "primary_case_count": len(primary_rows),
        "primary_case_ids": [row["sample_id"] for row in primary_rows],
        "primary_set_sha256": sha256_file(PROJECT_ROOT / A6_PRIMARY_SET_REL),
        "diagnostic_case_count": len(diagnostic_rows),
        "diagnostic_case_ids": [row["sample_id"] for row in diagnostic_rows],
        "diagnostic_set_sha256": sha256_text(canonical_json([sha256_file(PROJECT_ROOT / rel) for rel in A6_DIAGNOSTIC_SET_RELS])),
        "diagnostic_set_paths": A6_DIAGNOSTIC_SET_RELS,
        "stage7c_a6_lock_sha256": sha256_file(PROJECT_ROOT / STAGE7C_A6_DIR / "STAGE7C_A6_LOCK.json"),
...
def runner_protocol(accepted_commit: str) -> dict[str, Any]:
    return {
        ...
        "generation_contract": {
            "primary_cases": "12 locked Stage7C-A6 primary English column-conditioned cases",
            "diagnostic_cases": "36 Stage7C-A6 diagnostics deferred until after primary result is frozen and reviewed",
            ...
            "runner_file": "scripts/server/run_stage7e0_a6_english.py",
            "runtime_preflight_file": "scripts/server/preflight_runtime_stage7e0_a6.py",
...
if command -v conda >/dev/null 2>&1; then
  ...
  if ! conda env list | awk '{{print $1}}' | grep -qx Stage7E0_A6_uet_py312; then
    conda create -y -n Stage7E0_A6_uet_py312 python=3.12
  fi
  conda activate Stage7E0_A6_uet_py312
fi
...
python scripts/data/validate_stage7c_a6_atomic_domain_column_conditioned_protocol_freeze.py --stage-dir {STAGE7C_A6_DIR}
python scripts/data/validate_stage7e0_a6_english_preflight.py --stage-dir {STAGE_NAME}
...
python scripts/server/run_stage7e0_a6_english.py \\
...
python scripts/data/validate_stage7e0_a6_server_results.py --result-dir "$RESULT_ROOT"
...
def server_commands(accepted_commit: str) -> str:
    return f""": <<'Stage7E0_A6_MARKDOWN_DOC'
# Stage7E0-A6 English Atomic-Domain Column-Conditioned UET RTX4090 Commands
...
def validation_report(accepted_commit: str, mock_summary: dict[str, Any]) -> str:
    return f"""# Stage7E0-A6 English Atomic-Domain Column-Conditioned Real Generation Preflight Validation Report
...
primary_cases=12
diagnostic_cases={EXPECTED_DIAGNOSTIC_COUNT} after primary freeze and review only
...
def reviewer_readme(package_name: str, accepted_commit: str) -> str:
    return f"""# Stage7E0-A6 English Atomic-Domain Column-Conditioned Real Generation Preflight {PATCH_NAME}
...
Accepted Stage7C-A6 protocol commit: `{accepted_commit}`
"""


def build_stage(out_dir: Path, package_path: Path | None) -> dict[str, Any]:
    accepted_commit = frozen_accepted_stage7c_a6_commit()
    if out_dir.exists():
        shutil.rmtree(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    inputs = stage7c_a6_inputs(accepted_commit)
    protocol = runner_protocol(accepted_commit)
    write_json(out_dir / "STAGE7E0_A6_INPUT_MANIFEST.json", inputs)
    ...
    write_json(out_dir / "RUNNER_PROTOCOL_A6.json", protocol)
    write_json(out_dir / "PRIMARY_ACCEPTANCE_POLICY_A6.json", protocol["acceptance"])
    write_json(out_dir / "CONSTRAINT_INDEPENDENCE_AUDIT_A6.json", independence)
    ...
    artifact_names = [
        "STAGE7E0_A6_INPUT_MANIFEST.json",
        "RUNNER_PROTOCOL_A6.json",
        "PRIMARY_ACCEPTANCE_POLICY_A6.json",
        "CONSTRAINT_INDEPENDENCE_AUDIT_A6.json",
    ...
    lock = {
        ...
        "device_map": "auto",
        "max_memory": None,
        "constraint_independence_audit_sha256": sha256_file(out_dir / "CONSTRAINT_INDEPENDENCE_AUDIT_A6.json"),
        "model_called": False,
        "gpu_called": False,
    ...
    write_json(out_dir / "STAGE7E0_A6_LOCK.json", lock)
    summary = {
        "stage": STAGE_NAME,
        "patch": PATCH_NAME,
        "status": lock["status"],
...
def include_paths(stage_dir: Path) -> list[Path]:
    ...
    include_rel = [
        ...
        "Stage7C_A5_PRIMARY_GOLD_PROVENANCE_ERRATUM_PATCH0",
        "Stage7E0_A4_ENGLISH_CANDIDATE_SPAN_REAL_GENERATION_PREFLIGHT",
        "Stage7E0_A5_ENGLISH_COLUMN_CONDITIONED_REAL_GENERATION_PREFLIGHT",
        "Stage7B_A2_ENGLISH_CANDIDATE_SPAN_REFERENCE_AMENDMENT",
        "Stage7B_A3_ENGLISH_COLUMN_CONDITIONED_CANDIDATE_SELECTION_AMENDMENT",
        "Stage7B_A4_ENGLISH_ATOMIC_CANDIDATE_DOMAIN_AND_OMISSION_CUE_AMENDMENT",

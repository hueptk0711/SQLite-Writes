...
INFERENCE_LOCK = {
    ...
    "batching_policy": "fixed batch_size=1; resume keyed by sample_id",
}
CONFIGS = [
...
GENERATION_ARMS = {"direct", "j_fs", "original_mp_fs_plus", "d_g1_primary"}
...
def select_fresh_samples(
    ...
    diagnostic_groups = {
        str(row.get("source_group") or row.get("id") or row.get("sample_id") or "")
    ...
    for sample in samples:
        ...
        source_group = str(sample.get("source_group") or sample_id)
            ...
            rows.append(
                {
                    ...
                    "generation_arm": slug if slug in GENERATION_ARMS else "d_g1_primary",
                    "shares_raw_generation_with": "" if slug in GENERATION_ARMS else "d_g1_primary",
...
def overlap_audit(
    ...
    def groups(samples: list[dict[str, Any]]) -> set[str]:
        return {str(row.get("source_group") or row.get("id") or row.get("sample_id") or "") for row in samples}
...
Primary comparison: Original MP-FS+ vs D_G1.
Secondary ablations on the same vNext raw generation: D_ONLY, FULL, NO_C.
""",
    )
    write_text(
...
def validate_protocol(root: Path) -> dict[str, Any]:
    ...
    for sample_id, sample_rows in by_sample.items():
        ...
        generated = [row for row in sample_rows if row["generation_role"] == "generation"]
        if {row["method_slug"] for row in generated} != GENERATION_ARMS:
...
def main() -> int:
    ...
    run_lock_template = {
        ...
        "generation_arms": sorted(GENERATION_ARMS),
        "secondary_reprocesses_share_raw_generation_with": {
            "d_only_secondary": "d_g1_primary",
            "full_secondary": "d_g1_primary",
            "no_c_secondary": "d_g1_primary",
        },

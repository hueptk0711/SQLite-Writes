...
import pytest
import scripts.server.run_stage7e0_a6_english as stage7e0_runner
...
from scripts.data.build_stage7c_a6_atomic_domain_column_conditioned_protocol_freeze import canonical_json
from scripts.data.build_stage7e0_a6_english_preflight import PACKAGE_NAME, STAGE_NAME, build_stage
from scripts.data.validate_stage7e0_a6_english_preflight import validate
from scripts.data.validate_stage7e0_a6_server_results import validate as validate_server_result
from scripts.server.run_stage7e0_a6_english import (
...
def accepted_commit_for_tests() -> str:
    manifest_path = ROOT / STAGE_NAME / "STAGE7E0_A6_INPUT_MANIFEST.json"
    if manifest_path.is_file():
        return read_json(manifest_path)["accepted_stage7c_a6_commit"]
    if (ROOT / ".git").exists():
        return subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=ROOT, text=True).strip()
    lock_path = ROOT / STAGE_NAME / "STAGE7E0_A6_LOCK.json"
...
def test_Stage7E0_A6_uses_exact_column_conditioned_primary_rows() -> None:
    rows = load_stage7c_a6_rows(ROOT)
    assert len(rows) == 12
    assert all(row["sample_id"].startswith("stage7c_a6_primary_english_") for row in rows)
...
def test_column_conditioned_grammar_accepts_gold_and_rejects_unknown_refs() -> None:
    row = load_stage7c_a6_rows(ROOT)[0]
    ...
    first_span = next(value for value in row["label_side_expected"]["phase_o"]["column_span_refs"].values() if value != "OMIT")
    assert grammar.is_complete(gold)
    assert grammar.is_prefix(gold[:-1])
    assert not grammar.is_prefix(gold.replace(first_span, "SPAN_9999"))
...
def test_builder_and_validator_pass(tmp_path: Path) -> None:
    ...
    assert summary["status"] == "PASS_READY_FOR_REAL_A6_CONSTRAINED_PREFLIGHT"
...
def test_stage_lock_forbids_11_of_12_phase_m_and_gretel(tmp_path: Path) -> None:
    ...
    lock = read_json(stage_dir / "STAGE7E0_A6_LOCK.json")
    policy = read_json(stage_dir / "PRIMARY_ACCEPTANCE_POLICY_A6.json")
    protocol = read_json(stage_dir / "RUNNER_PROTOCOL_A6.json")
...
def test_clean_reviewer_zip_validator_passes(tmp_path: Path) -> None:
    if os.environ.get("STAGE7E0_A6_SKIP_NESTED_CLEAN_ZIP_FULL_PYTEST") == "1":
        return
    stage_dir = tmp_path / STAGE_NAME
    package_path = tmp_path / PACKAGE_NAME
    ...
    result = subprocess.run(
        [sys.executable, "scripts/data/validate_stage7e0_a6_english_preflight.py", "--stage-dir", STAGE_NAME],
        cwd=extract,
        text=True,
    ...
    full = subprocess.run(
        ...
        stderr=subprocess.PIPE,
        check=False,
        env=os.environ | {"STAGE7E0_A6_SKIP_NESTED_CLEAN_ZIP_FULL_PYTEST": "1"},
    )
    assert full.returncode == 0, full.stdout + full.stderr


def _expected_input_manifest_for_tests() -> dict:
    path = ROOT / STAGE_NAME / "STAGE7E0_A6_INPUT_MANIFEST.json"
    if path.is_file():
...
def _wrong_phase_o(row: dict) -> dict:
    ...
    for branch in row["runtime_constraints"]["phase_o_schema"].get("oneOf", [row["runtime_constraints"]["phase_o_schema"]]):
        if branch["properties"]["table_ref"].get("const") == phase_o["table_ref"]:
            ...
            return phase_o
    raise AssertionError("could not build wrong A6 phase_o")
...
def _write_server_result(result_dir: Path, *, protocol_valid: bool, pass_count: int, accepted_protocol_commit: str | None = None) -> None:
    ...
    accepted = accepted_protocol_commit or expected_input["accepted_stage7c_a6_commit"]
    write_json(
        result_dir / "run_manifest.json",
        {
            ...
            "git": {
                ...
                "frozen_accepted_protocol_commit": expected_input["accepted_stage7c_a6_commit"],
            },
            "stage7c_a6_inputs": {
                "stage7c_a6_lock_sha256": expected_input["stage7c_a6_lock_sha256"],
                "a6_prompt_spec_sha256": expected_input["phase_o_prompt_spec_sha256"],
                "a6_primary_set_sha256": expected_input["primary_set_sha256"],
                "a6_diagnostic_set_sha256": expected_input["diagnostic_set_sha256"],
            ...
            "phase_o_prompt_spec_path": "Stage7C_A6_ENGLISH_ATOMIC_DOMAIN_COLUMN_CONDITIONED_PROTOCOL_FREEZE/ATOMIC_DOMAIN_COLUMN_CONDITIONED_PROMPT_SPEC_A6_ENGLISH.json",
            "retry": 0,
            "repair": "none",
            "phase_o_max_new_tokens": 512,
...
def test_server_result_validator_rejects_fake_case_ids(tmp_path: Path) -> None:
    ...
    assert "primary_case_result_ids_must_equal_locked_A6_primary_ids" in report["evidence_failures"]
    assert "raw_generation_ids_must_equal_locked_A6_primary_ids" in report["evidence_failures"]
...
def test_server_result_validator_rejects_missing_raw_output(tmp_path: Path) -> None:
    ...
    report = validate_server_result(result_dir)
    assert report["evidence_integrity_status"] == "FAIL"
    assert any(failure.startswith("raw_generation_missing_required_keys:stage7c_a6_primary_english_001:raw_output") for failure in report["evidence_failures"])
...
def test_server_result_validator_rejects_raw_output_tamper_after_rehashed_summary(tmp_path: Path) -> None:
    ...
    report = validate_server_result(result_dir)
    assert report["evidence_integrity_status"] == "FAIL"
    assert "case_result_replay_mismatch:stage7c_a6_primary_english_001" in report["evidence_failures"]
    assert "primary_pass_count_must_match_replayed_case_rows" in report["evidence_failures"]
...
def test_server_result_validator_rejects_wrong_message_hash(tmp_path: Path) -> None:
    ...
    report = validate_server_result(result_dir)
    assert report["evidence_integrity_status"] == "FAIL"
    assert "raw_generation_messages_sha256_mismatch:stage7c_a6_primary_english_001" in report["evidence_failures"]

...
def test_atomic_dominance_suppresses_label_plus_identifier_but_keeps_atomic_child() -> None:
    ...
    assert reasons[broad.span_ref]["rule"] == "ATOMIC_DOMINATED_BROAD_SPAN"
    assert reasons[broad.span_ref]["dominant_child_text"] == "LOAN-842"
    assert child.span_ref not in reasons
    assert atomic_dominance_reason(candidates["mass 0.42"], inventory)["dominant_child_text"] == "0.42"
    assert atomic_dominance_reason(candidates["station_id 0xA5C0DE"], inventory)["dominant_child_text"] == "0xA5C0DE"
...
def test_atomic_dominance_does_not_drop_single_token_alphanumeric_values() -> None:
    ...
    assert atomic_dominance_reason(candidates["Q2"], inventory) is None
    assert atomic_dominance_reason(candidates["789B"], inventory) is None
    assert atomic_dominance_reason(candidates["S102"], inventory) is None
...
def test_build_and_validator_pass_on_728_design_train(tmp_path: Path) -> None:
    ...
    assert filtered["covered_assignment_count"] == 2249
    assert filtered["full_sample_covered_count"] == 721
...
def test_validator_rejects_threshold_tamper(tmp_path: Path) -> None:
    stage_dir = tmp_path / STAGE_NAME
    build_stage(stage_dir, raw_dir_or_skip())
    filtered_path = stage_dir / "ATOMIC_FILTERED_DOMAIN_AUDIT.json"
    ...
    assert "filtered_full_sample_representability_below_threshold" in report["failures"]
    assert "derived_manifest_hash_mismatch:ATOMIC_FILTERED_DOMAIN_AUDIT.json" in report["failures"]
